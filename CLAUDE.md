...
|-----|--------|-------|--------|
| 1 | Project setup + Supabase schema (12 tables + indexes) | `package.json`, `src/server.js`, `supabase/migrations/001_initial_schema.sql`, `.env.example` | NOT STARTED |
| 2 | Async webhook handler + idempotency + restaurant lookup | `src/webhooks/ghl.js`, `src/worker.js`, `src/db/queries/webhooks.js`, `src/db/queries/restaurants.js` | NOT STARTED |
| 3 | Hybrid intent classifier (regex + Claude) + conversation state | `src/bot/classifier.js`, `src/bot/conversation.js`, `src/services/claude.js` | NOT STARTED |
| 4 | Content pipeline (Claude copy + fal.ai image) + status machine | `src/content/generate.js`, `src/services/fal.js`, `src/db/queries/content.js` | NOT STARTED |
| 5 | WhatsApp delivery + approval flow + 15-min buffer + GHL publish | `src/content/publish.js`, `src/bot/handlers/approval.js`, `src/services/ghl.js` | NOT STARTED |
...
| 7 | Human review queue (HTML interface) | `review-queue/index.html`, `src/webhooks/review.js` | NOT STARTED |
...
### Database
- 12 tables total (see PRD v2.3 Section 14)
...
| Review queue UI | `review-queue/index.html` |
